...
from anketa import (get_report_date_end, get_report_date_start, get_report_incorrect,
                    get_report_start, get_api_key, get_client_id)
...
def main_bot():
    ...
    report = ConversationHandler(
        entry_points = [MessageHandler(Filters.regex('^(Ввести данные)$'), get_report_start)],
        states = {
            'client_id': [MessageHandler(Filters.text, get_client_id)],
